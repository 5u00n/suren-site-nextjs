...
const CONTACT_EMAIL = "surenhembram97@gmail.com";
...
function ContactSection() {
  ...
  function handleSubmit(e) {
    ...
    const subjectParam = encodeURIComponent(
      subject.trim() ? `[Contact] ${subject.trim()}` : "[Contact from portfolio]"
    );
    const bodyParam = encodeURIComponent(body);

    let mailtoUrl = `mailto:${CONTACT_EMAIL}?subject=${subjectParam}&body=${bodyParam}`;
    if (mailtoUrl.length > MAILTO_URL_MAX_LENGTH) {
      mailtoUrl = `mailto:${CONTACT_EMAIL}?subject=${subjectParam}&body=${encodeURIComponent(
    ...
    <section id="contact-section" className="w-full h-full md:px-16 sm:px-5 xs:px-5 bg-[#FFFFFF] dark:bg-transparent py-16 lg:rounded">
      <div className="w-full h-full flex flex-col gap-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <form onSubmit={handleSubmit} className="rounded-lg border bg-white dark:bg-gray-800/40 shadow-md transition-all hover:shadow-lg p-6">
